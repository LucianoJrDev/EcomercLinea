## Welcome everyone!

## About Linea

✨ Timeless Elegance

Discover the LINEA collection, where minimalist design meets contemporary sophistication. Each piece was created to highlight the beauty of simplicity, combining modern geometric shapes with impeccable craftsmanship.

Our earrings and bracelets are more than accessories—they are expressions of style, personality, and elegance. Inspired by architecture, art, and the harmony of lines, the pieces in the collection bring lightness and exclusivity to those who value unique details.

Whether for a special occasion or to elevate your everyday style, LINEA transforms every look into a statement of authenticity and refinement.

## What was added?

As you can see, the site is fully responsive, working on both computers and mobile devices. 

## Features:

- Shopping cart menu

- Add item as favorite

- Images with smooth and refined animation

- Search button working correctly + suggestions for best-selling products

- Product photos + photos of models wearing the product for a broad understanding of how the product would look on the wearer.

  <img width="1875" height="839" alt="image" src="https://github.com/user-attachments/assets/a3dd33d5-b5d3-444d-b437-22d0788dc3fa" />
<img width="1892" height="648" alt="image" src="https://github.com/user-attachments/assets/a493d1fd-d79d-4c82-9bd8-12d8dd3cc4fb" />
<img width="1861" height="297" alt="image" src="https://github.com/user-attachments/assets/e0b3d648-4a62-436c-95b7-71c533342ea9" />
<img width="393" height="851" alt="image" src="https://github.com/user-attachments/assets/6ce0a48f-b556-47ec-91c4-680b5fbf517d" />






## What technologies are used for this project?

This project is built with:

- Vite
- TypeScript
- React
- shadcn-ui
- Tailwind CSS

